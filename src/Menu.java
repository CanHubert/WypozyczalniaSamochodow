...
public class Menu {
    ...
    private static void optionRemoveUser(Database db){
        ...
        User user =db.getU().getUserbyID(id);
        try {db.getU().remove(user,db);
    ...
    private static void removeVehicle(Database db){
        ...
        Vehicle v= db.getV().getVehicleByID(id);
        try {db.getV().remove(v,db);
    ...
    private static void optionHireVehicle(Database db)throws RuntimeException{
        ...
        try {db.getH().HireAVehicle(idUser, idVeh, day,db);
            System.out.println("User ("+db.getU().getUserbyID(idUser).toString2()+") hired ("+db.getV().getVehicleByID(idVeh).toString2()+")");
    ...
    private static void optionReturnVehicle(Database db) throws RuntimeException{
        int idUser =InputInt("Insert User's ID");
        try{db.getH().returnVehicle(idUser,db);
    ...
    private static void optionDisplayUser(Database db){
        ...
        switch (i){
            case 1:{
                System.out.println(db.getU());
                Wait();
            }break;
            case 2:{
                System.out.println("Users with vehicles:");
                System.out.println(db.getU().printByVehicle(db,true));
                Wait();
            }break;
            case 3:{
                System.out.println("Users without vehicles:");
                System.out.println(db.getU().printByVehicle(db,false));
                Wait();
            }break;
            case 4:{
                int a = InputInt("Search by specific ID");
                try {
                    System.out.println(db.getU().printByID(a,db));
    ...
    private static void optionDisplayVehicles(Database db){
        ...
        switch (i){
            case 1:{
                ...
                System.out.println(db.getV().printByID()); Wait();
            }break;
            case 2:{
                System.out.println("Vehicles sorted by type:");
                System.out.println(db.getV().printByType()); Wait();
            }break;
            case 3:{
                int a = InputInt("Search by specific ID");
                try{
                    System.out.println(db.getV().printBySpecID(a,db)); Wait();
    ...
    switch(i){
        case 1:{System.out.println(db.getH());Wait();}break;
        case 2:{System.out.println(db.getH().PrintByStatus(true));Wait();}break;
        case 3:{System.out.println(db.getH().PrintByStatus(false));Wait();}break;
        case 0:{}break;
        default:
